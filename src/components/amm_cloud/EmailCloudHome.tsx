...
import { Email } from "../../interfaces/amm_cloud";
...
import { deleteEmailAmmCloud, getEmailAmmCloud } from "../helpers/ServiceAmmCloud";

export const EmailCloudHome = () => {
  ...
  const loadAmmApm = async (page: any, searchTxt: any) => {
    if (searchTxt !== "") {
      const respData = await getEmailAmmCloud(page, searchTxt);
      setInformation([...respData.data.results]);
    } else {
      const respData = await getEmailAmmCloud(page, "");
  ...
  const handleChange = (e: any) => {
    loadAmmApm(1, e.target.value);
  };

  const handleDelete = async (id: number) => {
    await deleteEmailAmmCloud(id);
    loadAmmApm(currenPage, "");
  };
  ...
  return (
    <>
      <div className="bg-emerald-600 h-auto from-gray-400 dark:bg-gray-600">
          ...
          {userInfo === true && (
            <div className="flex flex-col items-end sm:mr-12 -mt-8">
              <button
                ...
                className="bg-green-400 rounded-lg w-20 h-8 text-gray-800 mr-8 text-xs"
              >
                Crear
              </button>
            </div>
