...
import { ModalView } from "../amm_im/ModalView";
import InfiniteScroll from "react-infinite-scroll-component";
import { getUserId } from "../helpers/ServiceUser";
import { Amm_apm } from "../../interfaces/amm_apm";

export const RetiredCloudHome = () => {
  const [information, setInformation] = useState<Amm_cloud[]>([]);
  ...
  const loadAmmCloudFalse = async (page: any, searchTxt: any) => {
    if (searchTxt !== "") {
      const respData = await getAmmCloudFalse(page, searchTxt);
      setInformation([...respData.data.results]);
    } else {
      const respData = await getAmmCloudFalse(page, "");
  ...
  const handleChange = (e: any) => {
    ...
    loadAmmCloudFalse(1, e.target.value);
  };
  ...
  const handleDelete = async (id: number, oc_order: any) => {
    ...
    await updateAmmCloudId(id, resp.data);
    loadAmmCloudFalse(currenPage, "");
  };
            ...
            <table className="w-full text-xs text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {information?.map((info) => {
                  return (
                    <tr
                      className="bg-white border-b  hover:bg-green-200 text-gray-900 dark:bg-gray-900 dark:text-gray-200"
                      key={info.id}
                    >
                      {info.status === true ? (
                      ...
                      <td>
                      <p className="px-2 py-2 w-28 truncate ... capitalize">{info.details_query}</p>
                      </td>
                      <td>
