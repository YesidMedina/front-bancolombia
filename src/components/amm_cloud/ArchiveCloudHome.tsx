...
export const ArchiveCloudHome = () => {
  ...
  const loadAmmApmTrue = async (page: any, searchTxt: any) => {
    if (searchTxt !== "") {
      const respData = await getAmmCloudTrue(page, searchTxt);
      setInformation([...respData.data.results]);
    } else {
      const respData = await getAmmCloudTrue(page, "");
  ...
  return (
    <>
      <div className="bg-red-100 h-auto dark:bg-gray-800">
          ...
          <div className=" overflow-x-auto shadow-md sm:rounded-lg mt-4 mx-4">
            <table className="overflow-y-auto w-full text-xs text-left text-gray-500 dark:text-gray-400">
              <Table
                ...
                title2="Filial"
                title3="Servicio"
                title4="Item configuración"
                title5="Impacto"
                title6="Detalle query"
                title7="Correo"
                title8="OC"
              />
              <tbody>
                {information?.map((info, index) => {
                  return (
                    <tr
                      className="bg-white border-b  hover:bg-red-100 text-gray-900 dark:bg-gray-900 dark:text-gray-400"
                      key={index}
                    >
                      ...
                      <td>
                        <p className="px-2 py-2 w-28 truncate ...">{info.impact} </p>
                      </td>
                      <td>
                        <p className="px-2 py-2 w-28 truncate ...">
                          {" "}
                          ...
                          {userInfo === true && (
                            <>
                              <button
                                ...
                                onClick={() => navigate(`/update_cloud/${info.id}`)}
                              >
                                Editar
                              </button>
                              <button
