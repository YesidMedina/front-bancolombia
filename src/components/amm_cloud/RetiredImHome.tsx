...
import { getAmmCloud } from "../helpers/ServiceAmmCloud";
...
export const RetiredImHome = () => {
  ...
  const [table, setTable] = useState<Amm_cloud[]>([]);
  const [search, setSearch] = useState("");
  const [currenPage, setCurrenPage] = useState(0);

  const loadAmmIm = async () => {
    const respData = await getAmmCloud();
    ...
    const result = table.filter((element) => {
      if (
        ...
        element.item_configuration
          ...
          .toLowerCase()
          .includes(see.toLowerCase()) ||
        element.impact.includes(see.toLowerCase())
      ) {
        return element;
      }
    });
    ...
    <>
      <div className="bg-yellow-100 bg-gradient-to-r from-gray-400">
        ...
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-4 mx-16 ">
          ...
          <div className="-mt-8 p-12 relative flex flex-col items-center">
            <button
