...
import { getAmmApmFalse, getAmmApmId, updateAmmApmId } from "../helpers/ServiceAmmApm";
...
export const RetiredApmHome = () => {
  ...
  const loadAmmApmFalse = async (page: any, searchTxt: any) => {
    if (searchTxt !== "") {
      const respData = await getAmmApmFalse(page, searchTxt);
      setInformation([...respData.data.results]);
    } else {
      const respData = await getAmmApmFalse(page, "");
  ...
  const handleChange = (e: any) => {
    ...
    loadAmmApmFalse(1, e.target.value);
  };
