...
import { deleteEmailAmmApm, getEmailAmmApm } from "../helpers/ServiceAmmApm";
...
export const EmailApmHome = () => {
  ...
  const loadAmmApm = async (page: any, searchTxt: any) => {
    if (searchTxt !== "") {
      const respData = await getEmailAmmApm(page, searchTxt);
      setInformation([...respData.data.results]);
    } else {
      const respData = await getEmailAmmApm(page, "");
